...
def render(H, W, K, chunk=1024*32, rays=None, c2w=None, ndc=True,
    ...
    Args:
      ...
      W: int. 图像宽度.
      focal: float. 针孔相机的焦距.
      chunk: int. 表示同时处理的最大光线数量。 用于控制最大内存使用量。不影响最终结果。
      rays: 形状为[2, batch_size, 3]的数组. 批次中每个示例的光线起点和方向。
      c2w: 形状为[3, 4]的数组. 相机到世界空间的变换矩阵。
      ndc: bool. 如果为True，则表示射线在NDC坐标系中的原点、方向。
      near: 形状为[batch_size]的数组或浮点数. 射线的最近距离。
      far: 形状为[batch_size]的数组或浮点数. 射线的最远距离。
      use_viewdirs: bool. 如果为True，则使用模型中空间点的观察方向。
      c2w_staticcam: 形状为[3, 4]的数组. 若非None，则在使用其他c2w参数指定观察方向时，对该相机使用此变换矩阵。
    Returns:
      rgb_map: [batch_size, 3]. 射线的预测RGB值。
      disp_map: [batch_size]. 视差图。深度的倒数。
      acc_map: [batch_size]. 沿射线的累积不透明度（alpha）。
      extras: 一个字典，包含由render_rays()函数返回的所有内容。
    """
    if c2w is not None:
        # 从相机姿态生成光线
...
def render_path(render_poses, hwf, K, chunk, render_kwargs, gt_imgs=None, savedir=None, render_factor=0):
    ...
    hwf: 包含高度(H)、宽度(W)、焦距(focal)的元组
    K: 相机内参矩阵
    chunk: 分批处理的大小，用于控制内存使用
    render_kwargs: 渲染函数所需的其他参数
    gt_imgs: 可选的真实图像，用于计算指标
    savedir: 可选的保存目录，用于保存渲染结果
...
def create_nerf(args):
    ...
    # NDC设置
    # NDC（净偏差校正）仅适用于LLFF（低低频）风格的前向数据
...
def raw2outputs(raw, z_vals, rays_d, raw_noise_std=0, white_bkgd=False, pytest=False):
    """将模型预测的原始值转换为有意义的渲染输出
    Args:
        raw: [num_rays, num_samples along ray, 4]. 模型原始输出
        z_vals: [num_rays, num_samples along ray]. 沿每条射线的采样深度值
        rays_d: [num_rays, 3]. 射线方向
    Returns:
        rgb_map: [num_rays, 3]. 渲染的RGB图像
        disp_map: [num_rays]. 视差图（深度的倒数）
        acc_map: [num_rays]. 累积权重（总不透明度）
...
def render_rays(ray_batch,
                ...
                verbose=False,
                pytest=False):
    """体积渲染的核心函数，负责沿光线采样、查询模型、进行两阶段渲染（粗+细）
    Args:
      ray_batch: 数组[batch_size, ...]. 沿射线采样所需的所有信息，包括：射线起点、射线方向、最小距离、最大距离以及单位量级的观察方向。
      network_fn: function. 粗网络模型函数
      network_query_fn: 网络查询函数
      N_samples: int. 每条光线的粗采样点数
      retraw: bool. 是否返回原始预测值
      lindisp: bool. 是否在线性视差（逆深度）空间采样
      perturb: float, 0 or 1. 是否对采样点进行随机扰动（用于抗锯齿）
      N_importance: int. 细采样点数（0表示不使用细网络）
        这些样本仅传递给network_fine。
      network_fine: 细网络模型函数
      white_bkgd: bool.  是否使用白色背景
      raw_noise_std: 添加给密度预测的噪声标准差
      verbose: bool. 是否输出调试信息
      pytest:是否在测试模式下运行
    Returns:
      rgb_map: [num_rays, 3]. 细渲染的RGB颜色（最终输出）
      disp_map: [num_rays]. 细渲染的视差图
      acc_map: [num_rays]. 细渲染的累积不透明度
      raw: [num_rays, num_samples, 4]. 原始模型预测
      rgb0: See rgb_map. 粗渲染结果
      disp0: See disp_map. 粗渲染结果
      acc0: See acc_map. 粗渲染结果
      z_std: [num_rays]. （如果有细网络）细采样点深度的标准差，表示不确定性
    """
    # 解析光线批次
